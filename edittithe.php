...
$id = $_GET['id'];
$result = $conn->query("SELECT * FROM titherecords WHERE id = $id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE titherecords SET record_date=?, name=?, amount=? WHERE id=?");
    $stmt->bind_param("ssdi", $date, $name, $amount, $id);
    $stmt->execute();
...
<td><center><input type="text" id="nepali-datepicker" name="record_date" value="<?= $record['record_date'] ?>" required></center></td>
    <td><center><input type="text" name="name" value="<?= $record['name'] ?>" required></center></td>
